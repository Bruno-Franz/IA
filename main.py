...
def carregar_base_livros(path: str = "books_reviews.csv"):
    ...
    root. The default path was adjusted so ``rodar_tudo()`` works out of the
...
def treinar_flores_profunda(ds_train, ds_val, ds_test, num_classes):
    metrics = treinar_cnn_profunda(ds_train, ds_val, ds_test, num_classes)
    results_dt.append(
        {
            "dataset": "TF Flowers",
            "method": metrics["method"],
...
def rodar_tudo():
    ...
    if flowers is not None and tf is not None:
        ...
        treinar_flores_profunda(ds_train, ds_val, ds_test, info.features["label"].num_classes)
